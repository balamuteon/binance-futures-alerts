...
3.  **Сборка и запуск:**
    ...
    На Apple Silicon (`arm64`) дополнительная настройка для Kafka уже включена в compose-файлы:
    сервис `kafka` запускается как `linux/amd64`. По умолчанию используется
    `bitnamilegacy/kafka:3.7.0`, потому что исходный образ из `public.ecr.aws`
    может падать на pull с `EOF`, а `arm64`-вариант для этой линии тегов
    доступен не во всех registry одинаково стабильно.
